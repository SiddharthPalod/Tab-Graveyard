...
interface CatacombsPageProps {
  ...
  searchQuery: string;
  loading:     boolean;
  actions:     Pick<TabActions, 'resurrectTombstone' | 'shatterTombstone' | 'reviveTombstoneUrl'>;
}
...
export const CatacombsPage: React.FC<CatacombsPageProps> = ({
  ...
  // Track which tombstones are expanded to show their list of URLs
  ...
  // Filter tombstones by search query (matches tombstone title OR any URL/title inside)
  ...
  return (
    <div className="flex-1 overflow-y-auto overflow-x-hidden pr-1 space-y-2">
      {filteredTombstones.map((tomb) => {
        const isExpanded = expandedIds[tomb.id] ?? true; // Default expanded so URLs are immediately visible
